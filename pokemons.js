...
// Функция для отображения покемонов
function displayPokemons(pokemonArray) {
	const container = document.getElementById('pokemon-list');
	if (!container) {
		console.error("Элемент с id 'pokemon-list' не найден!");
		return;
	}
	...
	pokemonArray.forEach(pokemon => {
		...
		pokemonCard.innerHTML = `
			<div class="id">${pokemon.num}</div>
			<img src="${pokemon.img}" alt="${pokemon.name}">
			<h3>${pokemon.name}</h3>
			<p>${pokemon.type.join(' ')}</p>
			<p>Candy count: ${pokemon.candy_count || 'undefined'}</p>
...
// Кнопка "search" — фильтрует покемонов по имени
